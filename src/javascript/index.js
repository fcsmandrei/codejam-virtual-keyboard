...
keyboard.addEventListener('mousedown', (event) => {
    ...
    let cas = target.closest('.case');       
    let key = target.closest('.key');

    
    if(!key) return;
       
    if(31 < cas.textContent.charCodeAt() && 62 > cas.textContent.charCodeAt() || 1071 < cas.textContent.charCodeAt() && 1106 > cas.textContent.charCodeAt()) {
    ...
    if(cas.textContent === 'Shift') {
        keyboard.querySelectorAll('.down').forEach(i => i.style.display = 'none');
        keyboard.querySelectorAll('.up').forEach(i => {
...
keyboard.addEventListener('mouseup', (event) => {
    ...
    if(!cas) return;
    if(cas.textContent === 'Shift') {
        keyboard.querySelectorAll('.down').forEach(i => i.style.display = 'block');
        keyboard.querySelectorAll('.up').forEach(i => i.style.display = 'none');
    }
...
document.addEventListener('keydown', e => {
    ...
    keys.forEach(i => {
       if(i.textContent.includes(e.key)) {
